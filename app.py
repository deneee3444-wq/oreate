...
class OreateBot:
    ...
    def get_user_info(self, referer: str | None = None) -> dict:
        ...
        if grant and grant.get("pointGrant"):
            ...
            print(f"[bonus] pointGrant={total}  →  {msg}", flush=True)
        return data
    ...
    def _parse_sse(self, response) -> list[dict]:
        ...
        for line in response.iter_lines(decode_unicode=True):
            ...
            try:
                ...
                chunk_index += 1
                try:
                    print(f"[sse-chunk #{chunk_index}] {json.dumps(data, ensure_ascii=False)}", flush=True)
                except Exception:
                    print(f"[sse-chunk #{chunk_index}] {data}", flush=True)
                events.append(data)
            except json.JSONDecodeError:
                print(f"[sse-chunk-raw] {line}", flush=True)
                continue
        return events
    ...
    if bot is None:
        with bot_lock:
            if bot is None:
                ...
                try:
                    print("[+] Otomatik ilk kayıt başlatılıyor (80 kredi için)...", flush=True)
                    res = bot.run()
                    time.sleep(1.0)
                    bot.trigger_daily_bonus("signup_extra")
                    print(f"[+] Otomatik ilk kayıt başarılı: {res.get('email')}", flush=True)
                except Exception as e:
                    print(f"[-] Otomatik ilk kayıt hatası: {e}", flush=True)
    return bot
...
def background_generate(task_id, mode, prompt, model, resolution, ratio, duration, audio, ai_type, scene, in_memory_files):
    ...
    try:
        ...
        if mode == 'image':
            ...
            print(f"[generate_image] task={task_id} chat_id={chat_id}", flush=True)
            ...
            print(f"\n[generate_image] task={task_id} total_events={len(events)}", flush=True)
            try:
                print(json.dumps(events, ensure_ascii=False, indent=2), flush=True)
            except Exception:
                print(events, flush=True)
            result_url = OreateBot.extract_result_url(events)
            print(f"[generate_image] task={task_id} result_url={result_url}\n", flush=True)
        else:
            update_log("Video oturumu (chat) oluşturuluyor...")
            chat_id = b.create_chat("aiVideo")
            print(f"[generate_video] task={task_id} chat_id={chat_id}", flush=True)
            ...
            print(f"\n[generate_video] task={task_id} total_events={len(events)}", flush=True)
            try:
                print(json.dumps(events, ensure_ascii=False, indent=2), flush=True)
            except Exception:
                print(events, flush=True)
            result_url = OreateBot.extract_result_url(events)
            print(f"[generate_video] task={task_id} result_url={result_url}\n", flush=True)
        ...
        if result_url:
            ...
            print(f"[generate] task={task_id} HATA: {err_msg}", flush=True)
        ...
        print(f"[generate] task={task_id} EXCEPTION: {e}", flush=True)
        with tasks_lock:
            if task_id in tasks:
                tasks[task_id]['status'] = 'Hata'
